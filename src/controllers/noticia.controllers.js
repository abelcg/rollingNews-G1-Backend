...
noticiaCtrl.listaNoticias = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({ mensaje: "Error al buscar las noticias solicitadas" });
  }
};
...
noticiaCtrl.noticiasXcategoria = async (req, res) => {
   ...
   try {
    const noticiasXcategoria = await Noticia.find({ categoria })
      .sort({ sort, sort: order  });
    res.status(200).json(noticiasXcategoria);
  } catch (error) {
    console.log(error);
    res
      .status(404)
